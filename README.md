...
Telegram-бот: отправляешь **анимированный** стикер — получаешь `.tgs` внутри `.zip`.

Обычные (`.webp`) и видео-стикеры (`.webm`) отклоняются: у них нет `.tgs`.

Файл отдаётся в zip, потому что «голый» `.tgs` Telegram часто показывает как стикер, а не как документ для скачивания.
...
5. В Telegram напиши боту `/start` и пришли анимированный стикер.
