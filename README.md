...
- Широкая панель по умолчанию (1000×700dp) — удобнее читать длинные пути и списки
...
### Если пункта «Wireless debugging» нет вообще (bootstrap через OTG)

На части сборок Horizon OS в параметрах разработчика нет самого пункта Wireless Debugging.
Без него нужен один разовый shell-сеанс с *любого* другого устройства — ПК/Mac, либо
Android-телефон с OTG-кабелем USB-C→USB-C и приложением
[aShellYou](https://github.com/DP-Hridayan/aShellYou) (open-source, режим OTG ADB):

1. Включи **USB debugging** в параметрах разработчика на очках (этот пункт есть всегда,
   в отличие от Wireless debugging).
2. Подключи очки кабелем к телефону/ПК, получи shell (в aShellYou: раздел OTG →
   подтверди «Allow USB debugging» на очках).
3. Выполни в шелле:
   ```
   ...
   start adbd
   ```
4. Отключи кабель. В VR CONTROLER: кнопка **«Через OTG»** на карточке Android/data →
   **«Подключить (5555)»**. Дальше приложение работает полностью автономно, кабель/телефон
   больше не нужны (пока не сбросишь данные приложения или не перезагрузишь очки — тогда
   шаги 1–3 повторяются).

Это тот же классический `adb tcpip`-режим, что и при обычном USB-подключении к ПК — просто
роль ПК на один раз играет телефон.
