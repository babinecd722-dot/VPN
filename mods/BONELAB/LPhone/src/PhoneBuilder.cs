...
namespace LPhone
{
    ...
    internal static class Phone
    {
        ...
        // 371x800. Подъём до 742x1600 стоил вчетверо больше на КАЖДЫЙ тап:
        // 13.6 МБ трафика памяти на кадр вместо 3.4 — отсюда и были фризы.
        // Выигрыша в чёткости при этом почти нет: телефон на вытянутой руке
        // занимает около 240 пикселей дисплея Quest, то есть даже 371 текселя
        // с запасом хватает. Нечитаемость была от перевёрнутой картинки.
