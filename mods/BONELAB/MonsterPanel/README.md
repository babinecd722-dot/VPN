...
- **Kill Aura** — подстраница-настройки урона по игрокам, **без наведения и крестиков**:
  ...
  - **Lightning FX (all see)** — **сетевая** молния: спавним настоящий электро-объект на
    цель через `NetworkAssetSpawner`, поэтому её видят **все** (у других мода нет — увидят
    только то, что синхронит сам Fusion). Оптимизировано: один удар за залп (round-robin по
    целям), авто-удаление каждого объекта по таймеру (`Despawn`), кап одновременных — не лагает
    и не засоряет лобби. Требует активный сервер и наличие электро-крейта в реестре (ищем по
    названию: lightning/electric/tesla/shock/…). Если крейт не найден — смотри лог;
