...
- **Kill Aura** — подстраница-настройки урона по игрокам, **без наведения и крестиков**:
  ...
  - **Lightning FX** — молния бьёт **ровно в игрока** (грудь) при убийстве. Локальный визуал
    (видишь ты): фикс-пул болтов (без Instantiate/Destroy в кадре, без GC), удары троттлятся —
    не лагает. Сетевую молнию убрал: единственный «lightning»-крейт в реестре — это пушка
    (падает предметом рядом, не удар) + частый спавн ловит ошибки LabFusion (дубли entity id);
