...
## Teleport / Bring (2.30.48 — Fusion 0.2.x peer mesh)

Fusion **0.2.0+** clients never `AcceptConnection` (only the host does). Direct
`EosDirectSend` Bring packets therefore never reached other players.

**2.30.48** fixes the root cause and hardens delivery:

- **Peer mesh** — client P2P uses host-style Accept/Established notifications
- **Bring** multi-path: mesh open → forged host `PlayerRepTeleport` bursts →
  ToServer ToTarget relay → PermissionCommand (+ forged `TELEPORT_TO_THEM`) →
  timed retries (0.2 / 0.5 / 1.0s)
- Wire format matches stock host teleports (`EncodePosition` via `Serialize`,
  `Sender=0`, client channel)
- **Teleport to player** — clear rigidbody velocities, then local teleport + force
- Host Bring stays stock `SendPlayerTeleport` (burst)

## Teleport / Bring (legacy notes)

Fusion **≤0.1.x**: client Bring used `MessageSender.SendFromServer` (no IsHost gate).
Fusion **0.2.0**: `SendFromServer` is host-only — MonsterPanel uses **`EosDirectSend`** (`Runtime.P2P.Sender`).
