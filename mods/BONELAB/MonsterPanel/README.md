...
- **Kill Aura** — макс. урон **всем игрокам рядом** (радиус ~15 м), автоматически, **без наведения и крестиков**. Идёт по списку `NetworkPlayer.Players`, бьёт раз в 0.25 с по всем частям тела через тот же путь, что и Monster Damage.
...
- **Disarm** — у **всех игроков рядом** (радиус ~15 м) раз в 0.5 с **вырывает оружие** силой, **без наведения**. Физика, не урон → чужое бессмертие не мешает. Тела игроков не трогаются, только отдельные предметы.
